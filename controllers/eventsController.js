...
exports.createEvents = async (req, res) => {
  try {
    const { title, shortdescription, file, eventsdatetime } = req.body;
    ...
    await pool.query(
      "INSERT INTO events (id,title,shortdescription,fileid,fileurl,createdat,eventsdatetime) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *",
      [
        id,
        title.toLowerCase(),
        shortdescription,
...
exports.updateEvents = async (req, res) => {
  try {
    ...
    if (file) {
      ...
      await pool.query(
        "UPDATE events SET fileid = $1, fileurl = $2, title = $3, shortdescription = $4, eventsdatetime = $5 WHERE id = $6",
        [myCloud.public_id, myCloud.secure_url, title, shortdescription, eventsdatetime, id]
      );
    } else {
      await pool.query(
        "UPDATE events SET title = $1,shortdescription = $2, eventsdatetime = $3 WHERE id = $4",
        [title, shortdescription, eventsdatetime, id]
      );
    }
    res.status(200).json({
      success: true,
